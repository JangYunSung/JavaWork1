...
SELECT a.NAME "교수명", a.HIREDATE "입사일", d.DNAME "학과명"
FROM T_PROFESSOR a ,T_PROFESSOR b, T_DEPARTMENT d
WHERE a.HIREDATE > b.HIREDATE AND d.DEPTNO =b.DEPTNO 
AND b.NAME = '송도권'
;
-- #7105) 연습
...
SELECT dcode FROM T_DEPT2 WHERE AREA IN '서울지사';

SELECT EMPNO , NAME , DEPTNO 
FROM T_EMP2
WHERE DEPTNO IN (SELECT dcode FROM T_DEPT2 WHERE AREA IN '서울지사')
;


SELECT PAY FROM T_EMP2 WHERE post LIKE '과%';

SELECT NAME "이름" , POST "직급" , TO_CHAR(pay, '999,999,999') || '원' "연봉"
FROM T_EMP2 
WHERE PAY >ANY (SELECT PAY FROM T_EMP2 WHERE post LIKE '과%')
;

SELECT WEIGHT FROM T_STUDENT WHERE GRADE IN 4
;

SELECT name "이름" , GRADE "학년" , WEIGHT "몸무게"
FROM T_STUDENT 
WHERE WEIGHT <ALL (SELECT WEIGHT FROM T_STUDENT WHERE GRADE IN 4);
